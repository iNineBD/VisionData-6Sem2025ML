"""
API FastAPI usando funções compartilhadas de feature_engineering.py
Muito mais simples e sem duplicação de código!
"""
import sys
import os
...
def predict_future(model, scaler, historical_data, days=30):
    ...
    for i in range(1, days + 1):
        ...
        hist_df = pd.concat([
            hist_df,
            pd.DataFrame([{'date': future_date, 'ticket_count': pred}])
        ], ignore_index=True)
...
async def health_check():
    ...
    return {
        "status": "healthy" if all([loaded_model, loaded_scaler, loaded_data is not None]) else "unhealthy",
...
async def model_info():
    ...
    return {
        ...
        "date_range": {
            "start": loaded_data['date'].min().strftime('%Y-%m-%d') if loaded_data is not None else None,
            "end": loaded_data['date'].max().strftime('%Y-%m-%d') if loaded_data is not None else None
        }
    }
...
async def get_forecast(days: int = 30, historical_days: int = 90):
    ...
    try:
        ...
        predictions_df = predict_future(loaded_model, loaded_scaler, loaded_data, days=days)
        ...
        return ForecastResponse(
            ...
            metadata={
                ...
                "last_historical_date": historical_df['date'].max().strftime('%Y-%m-%d'),
                "first_prediction_date": predictions_df['date'].min().strftime('%Y-%m-%d'),
                "last_prediction_date": predictions_df['date'].max().strftime('%Y-%m-%d'),
