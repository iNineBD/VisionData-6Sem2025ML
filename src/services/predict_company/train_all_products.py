...
def main():
    ...
    for prod in top_products:
        ...
        best = None
        with mlflow.start_run(run_name=prod):
            if sarimax:
                mlflow.log_metrics(
                    {
            ...
            if sarimax and lgbm:
                if sarimax["mse"] <= lgbm["mse"]:
                    best = sarimax["model"]
                    print(
                        f"  ✓ SARIMAX é o melhor para {prod} (MSE={sarimax['mse']:.2f} < {lgbm['mse']:.2f})"
                    )
                    mlflow.sklearn.log_model(
                        sarimax["model"],
                        "model",
                        registered_model_name=f"{prod}_SARIMAX",
                    )
                else:
                    best = lgbm["model"]
                    print(
                        f"  ✓ LightGBM é o melhor para {prod} (MSE={lgbm['mse']:.2f} < {sarimax['mse']:.2f})"
                    )
                    mlflow.sklearn.log_model(
                        lgbm["model"], "model", registered_model_name=f"{prod}_LGBM"
                    )
            elif sarimax:
                best = sarimax["model"]
                print(f"  ✓ Apenas SARIMAX treinado para {prod}")
                mlflow.sklearn.log_model(
                    sarimax["model"], "model", registered_model_name=f"{prod}_SARIMAX"
                )
            elif lgbm:
                best = lgbm["model"]
                print(f"  ✓ Apenas LightGBM treinado para {prod}")
                mlflow.sklearn.log_model(
                    lgbm["model"], "model", registered_model_name=f"{prod}_LGBM"
                )
            else:
