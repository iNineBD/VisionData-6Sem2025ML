...
def run_pipeline(csv_path: str, group_col: str):
    ...
    for item in top_values:
        ...
        if existing_models:
            ...
            if model_name == "LightGBM":
                ...
                for dt in future_index:
                    feats = {}
                    for lag in [1, 7, 14, 30]:
                        feats[f"lag_{lag}"] = tmp_series.get(
                            dt - pd.Timedelta(days=lag), 0
                        )
                    for w in [7, 30]:
                        vals = [
        ...
        best_model, best = (
            ("SARIMAX", sar) if score_sar < score_lgb else ("LightGBM", lgbm)
        )
    ...
    final_summary = [
        {
            ...
            **v,
            "total_next30": (
                int(round(v["total_next30"])) if v.get("total_next30") else None
            ),
